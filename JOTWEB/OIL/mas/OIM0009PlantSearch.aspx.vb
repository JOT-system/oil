...
Public Class OIM0009PlantSearch
    Inherits Page
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = OIM0009WRKINC.MAPIDS

        WF_PLANTCODE_CODE.Focus()
        WF_FIELD.Value = ""
    ...
    Protected Sub WW_MAPValueSet()
        ...
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.SUBMENU Then         'メニューからの画面遷移
            ...
            Master.GetFirstValue(work.WF_SEL_PLANTCODE.Text, "PLANTCODE", WF_PLANTCODE_CODE.Text)       '基地コード
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0009L Then   '実行画面からの遷移
            '画面項目設定処理
            WF_CAMPCODE.Text = work.WF_SEL_CAMPCODE.Text            '会社コード
            WF_ORG.Text = work.WF_SEL_ORG.Text                      '組織コード
            WF_PLANTCODE_CODE.Text = work.WF_SEL_PLANTCODE.Text     '基地コード
        End If

        '基地コードを入力するテキストボックスは数値(0～9)のみ可能とする。
        Me.WF_PLANTCODE_CODE.Attributes("onkeyPress") = "CheckNum()"

        '○ RightBox情報設定
        rightview.MAPIDS = OIM0009WRKINC.MAPIDS
        rightview.MAPID = OIM0009WRKINC.MAPIDL
        ...
        CODENAME_get("PLANTCODE", WF_PLANTCODE_CODE.Text, WF_PLANTCODE_NAME.Text, WW_DUMMY)     '基地コード
    ...
    Protected Sub WF_ButtonDO_Click()
        ...
        '○ 入力文字置き換え(使用禁止文字排除)
        Master.EraseCharToIgnore(WF_PLANTCODE_CODE.Text)     '基地コード
        ...
        work.WF_SEL_PLANTCODE.Text = WF_PLANTCODE_CODE.Text     '基地コード
    ...
    Protected Sub WW_Check(ByRef O_RTN As String)
        ...
        '○ 単項目チェック
        '基地コード
        Master.CheckField(WF_CAMPCODE.Text, "PLANTCODE", WF_PLANTCODE_CODE.Text, WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
        If isNormal(WW_CS0024FCHECKERR) Then
            '存在チェック
            If WF_PLANTCODE_CODE.Text <> "" Then
                CODENAME_get("PLANTCODE", WF_PLANTCODE_CODE.Text, WF_PLANTCODE_NAME.Text, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
                    Master.Output(C_MESSAGE_NO.NO_DATA_EXISTS_ERROR, C_MESSAGE_TYPE.ERR, "基地コード : " & WF_PLANTCODE_CODE.Text, needsPopUp:=True)
                    WF_PLANTCODE_CODE.Focus()
                    O_RTN = "ERR"
                    Exit Sub
                End If
    ...
    Protected Sub WF_ButtonSel_Click()
        ...
        Select Case WF_FIELD.Value
            Case "WF_PLANTCODE"          '基地コード
                WF_PLANTCODE_CODE.Text = WW_SelectValue
                WF_PLANTCODE_NAME.Text = WW_SelectText
                WF_PLANTCODE_CODE.Focus()
        End Select
