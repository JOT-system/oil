...
Public Class MP0009ActualTraction
    ...
    Private Function GetDownloadListData(sqlCon As SqlConnection) As DataTable
        ...
        '    .AppendLine("     , ODR.OTTRANSPORTFLG                   AS OTTRANSPORTFLG ")
        '    .AppendLine("     , (CASE ODR.OTTRANSPORTFLG ")
        '    .AppendLine("         WHEN 1 THEN SUM(ODR.CARSNUMBER) ")
        '    .AppendLine("         WHEN 2 THEN SUM(ODR.CARSNUMBER) ")
        '    .AppendLine("         ELSE 0 ")
        '    .AppendLine("     END)                                   AS CARSNUMBER ")
        '    .AppendLine("   , ISNULL(MAX(CONVERT(INT, ODR.LINE)), 0) AS LINE ")
        '    .AppendLine(" FROM ( ")
        '    .AppendLine("     SELECT ")
        ...
        '    .AppendLine("             , ISNULL(DTL.ACTUALLODDATE, ODR.LODDATE) AS LODDATE ")
        '    .AppendLine("             , ISNULL(DTL.ACTUALDEPDATE, ODR.DEPDATE) AS DEPDATE ")
        '    .AppendLine("             , DTL.SHIPPERSCODE ")
        '    .AppendLine("             , (CASE ")
        '    .AppendLine("                 WHEN ODR.CONSIGNEECODE = '54' ")
        '    .AppendLine("                 THEN DTL.OTTRANSPORTFLG ")
        '    .AppendLine("                 ELSE  ")
        '    .AppendLine("                     (CASE TRA.TRAINCLASS ")
        '    .AppendLine("                         WHEN 'J' THEN 2 ")
        '    .AppendLine("                         ELSE 1 ")
        '    .AppendLine("                     END) ")
        '    .AppendLine("             END) AS OTTRANSPORTFLG ")
        '    .AppendLine("             , DTL.LINE ")
        '    .AppendLine("             , DTL.CARSNUMBER ")
        '    .AppendLine("         FROM ")
        '    .AppendLine("             OIL.OIT0002_ORDER ODR ")
        '    .AppendLine("             INNER JOIN OIL.OIT0003_DETAIL DTL ")
        '    .AppendLine("                 ON  ODR.ORDERNO = DTL.ORDERNO ")
        '    .AppendLine("             INNER JOIN oil.OIM0007_TRAIN TRA ")
        '    .AppendLine("                 ON  ODR.OFFICECODE = TRA.OFFICECODE ")
        '    .AppendLine("                 AND ODR.TRAINNO = TRA.TRAINNO ")
        '    .AppendLine("                 AND (CASE ")
        '    .AppendLine("                         WHEN DTL.STACKINGFLG = 1 THEN 'T' ")
        '    .AppendLine("                         ELSE 'N' ")
        '    .AppendLine("                     END) = TRA.TSUMI ")
        '    .AppendLine("                 AND ODR.DEPSTATION = TRA.DEPSTATION ")
        '    .AppendLine("                 AND ODR.ARRSTATION = TRA.ARRSTATION ")
        '    .AppendLine("                 AND TRA.DEFAULTKBN = 'def' ")
        '    .AppendLine("                 AND TRA.DELFLG <> '1' ")
        '    .AppendLine("         WHERE ")
        ...
        '    .AppendLine("         AND LH.SHIPPERCODE = ODR.SHIPPERSCODE ")
        '    .AppendLine(" WHERE ")
        ...
        With sqlStat
            ...
            .AppendLine("     , ODR.SHIPPERSCODE                       AS SHIPPERCODE ")
            .AppendLine("     , ODR.ARRSTATION                         AS ARRSTATIONCODE ")
            .AppendLine("     , ODR.TRAINNO                            AS TRAINNO ")
            .AppendLine("     , FORMAT(ODR.LODDATE, 'yyyy/MM/dd')      AS LODDATE ")
            .AppendLine("     , FORMAT(ODR.DEPDATE, 'yyyy/MM/dd')      AS DEPDATE ")
            .AppendLine("     , ODR.OTTRANSPORTFLG                     AS OTTRANSPORTFLG ")
            .AppendLine("     , SUM(ODR.CARSNUMBER)                    AS CARSNUMBER ")
            .AppendLine("     , ISNULL(MAX(CONVERT(INT, ODR.LINE)), 0) AS LINE ")
            .AppendLine(" FROM ( ")
            .AppendLine("     SELECT ")
            .AppendLine("         ODR.OFFICECODE ")
            ...
            .AppendLine(" ) ODR ")
            .AppendLine(" WHERE ")
            .AppendLine(" 	ODR.OFFICECODE = @OFFICECODE ")
            .AppendLine(" AND ( ")
            .AppendLine(" 	ODR.LODDATE BETWEEN @BEGINDATE AND @ENDDATE OR ")
            .AppendLine(" 	ODR.DEPDATE BETWEEN @BEGINDATE AND @ENDDATE ")
            .AppendLine(" ) ")
            .AppendLine(" GROUP BY ")
            .AppendLine("     ODR.OFFICECODE ")
            .AppendLine("     , ODR.SHIPPERSCODE ")
            .AppendLine("     , ODR.ARRSTATION ")
            .AppendLine("     , ODR.TRAINNO ")
            .AppendLine("     , ODR.LODDATE ")
            .AppendLine("     , ODR.DEPDATE ")
