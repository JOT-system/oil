...
Public Class OIT0004OilStockCreate
    ...
    Protected Sub WW_MAPValueSet(Optional setConsignee As String = "", Optional SetConsigneeName As String = "")
        ...
        Using sqlCon = CS0050SESSION.getConnection
            ...
            If dispDataObj.HasMoveInsideItem Then
                ...
                dispDataObj = EditEmptyTurnCarsNumExtra(sqlCon, dispDataObj)
                '当日の日付について受入数を受注より取得
    ...
    Protected Sub WF_ButtonGETEMPTURN_Click()
        ...
        Using sqlCon = CS0050SESSION.getConnection
            ...
            If dispValues.ShowSuggestList = False Then
                dispValues = EditEmptyTurnCarsNum(sqlCon, dispValues, True, True)
    ...
    Private Function GetTargetTrain(sqlCon As SqlConnection, salesOffice As String, shipper As String, consignee As String, Optional targetTrainList As Dictionary(Of String, TrainListItem) = Nothing) As Dictionary(Of String, TrainListItem)
        Try
            ...
            If salesOffice = "012402" AndAlso consignee = "40" Then
                ...
                sqlStr.AppendLine("         ) ")
                sqlStr.AppendLine("       OR (     FX.CAMPCODE   = '011203'")
                sqlStr.AppendLine("            AND FX.VALUE5     =  @CONSIGNEECODE")
                sqlStr.AppendLine("            AND TR.OFFICECODE = '011203'")
                sqlStr.AppendLine("            AND TR.TRAINNO    = '5461'")
                sqlStr.AppendLine("         ) ")
                sqlStr.AppendLine("        ) ")
            ...
            Else
                ...
                sqlStr.AppendLine("   AND TR.DELFLG     = @DELFLG")
            End If
            sqlStr.AppendLine(" ORDER BY TR.OFFICECODE DESC,TR.ZAIKOSORT,TR.TRAINNO,TR.TSUMI")
            Using sqlCmd As New SqlCommand(sqlStr.ToString, sqlCon)
                ...
                Using sqlDr As SqlDataReader = sqlCmd.ExecuteReader()
                    ...
                    While sqlDr.Read
                        ...
                        '三重塩浜-JONET松本特殊処理:5461列車枠の生成 表示は列車番号ではなく富士石油
    ...
    Private Function GetTargetOilType(sqlCon As SqlConnection, salesOffice As String, consignee As String, shipper As String, Optional printMonth As String = "") As Dictionary(Of String, OilItem)
        ...
        sqlStr.AppendLine("      ,format(isnull(OTM.ORDERFROMDATE,convert(date,'1900/1/1')),'yyyy/MM/dd') AS ORDERFROMDATE")
        sqlStr.AppendLine("      ,format(isnull(OTM.ORDERTODATE,  convert(date,'9999/12/31')),'yyyy/MM/dd') AS ORDERTODATE")
        sqlStr.AppendLine("  FROM OIL.VIW0001_FIXVALUE FV")
        sqlStr.AppendLine("  LEFT JOIN OIL.OIM0030_OILTERM OTM ")
        sqlStr.AppendLine("         ON OTM.OFFICECODE     = @CAMPCODE")
        sqlStr.AppendLine("        AND OTM.SHIPPERCODE    = @SHIPPERSCODE")
        sqlStr.AppendLine("        AND OTM.OILCODE        = FV.KEYCODE")
        sqlStr.AppendLine("        AND OTM.SEGMENTOILCODE = FV.VALUE2")
        sqlStr.AppendLine("        AND OTM.CONSIGNEECODE  = @CONSIGNEE")
        sqlStr.AppendLine("        AND OTM.DELFLG         = @DELFLG")
        ...
        Using sqlCmd As New SqlCommand(sqlStr.ToString, sqlCon)
            ...
            Using sqlDr As SqlDataReader = sqlCmd.ExecuteReader()
                While sqlDr.Read
                    ...
                    orderFromDate = Convert.ToString(sqlDr("ORDERFROMDATE"))
                    orderToDate = Convert.ToString(sqlDr("ORDERTODATE"))
    ...
    ''' <returns></returns>
    ''' <remarks>受入予定数を設定当日のみ取得
    ''' TODO :: 川崎の足しこみ！！！</remarks>
    Private Function GetReciveFromOrder(sqlCon As SqlConnection, dispData As DispDataClass) As DispDataClass
        ...
        Dim qdataVal = From itm In dispData.StockDate
                       ...
                       Where itm.Value.ItemDate.ToString("yyyy/MM/dd") = Now.ToString("yyyy/MM/dd")
            ...
            Using sqlDr As SqlDataReader = sqlCmd.ExecuteReader()
                If sqlDr.HasRows Then
                    ...
                    While sqlDr.Read
                        ...
                        kawasakiVol = 0
                        If dispData.SalesOffice = "011203" AndAlso {"30", "54"}.Contains(dispData.Consignee) Then
                            kawasakiVol = dispData.GetSummarySuggestValue(targetDate, oilCode, True)
                        End If
                        With retVal.StockList(oilCode)
                            If .StockItemList.ContainsKey(targetDate) Then
                                recvVal = Decimal.Parse(Convert.ToString(sqlDr("AMOUNT"))) + kawasakiVol
    ...
    Private Function GetPrintTrainAmount(sqlCon As SqlConnection, dispData As DispDataClass) As DispDataClass
        ...
        Else
            ...
            sqlStat.AppendLine("     , ISNULL(SUM(CASE WHEN (ODR.TRAINNO IN ('83') AND DTL.AFTERRETURNFLG = '1') OR (ODR.TRAINNO = '81' AND DTL.FIRSTRETURNFLG <> '1') THEN DTL.CARSAMOUNT ELSE 0 END),0) AS AMOUNT3 ")
    ...
    ''' <param name="dispData"></param>
    ''' <returns></returns>
    Private Function EditEmptyTurnCarsNum(sqlCon As SqlConnection, dispData As DispDataClass, Optional isGetAlldays As Boolean = False, Optional isOtGetAll As Boolean = False) As DispDataClass
        'これもしかすると在庫テーブルから先週の払出量かと
        'オーダーは受入数な気がする(Consigneeしかないので)
        Dim sqlStr As New StringBuilder
        Dim retVal = dispData
        ...
        sqlStr.AppendLine(" WHERE ODR.LODDATE   BETWEEN @DATE_FROM AND @ADATE_TO")
        'sqlStr.AppendLine("   AND ODR.OFFICECODE      = @OFFICECODE")
        If dispData.SalesOffice = "012402" AndAlso dispData.Consignee = "40" Then
            sqlStr.AppendLine("   AND (   (ODR.OFFICECODE      = @OFFICECODE)")
            sqlStr.AppendLine("        OR (ODR.OFFICECODE      = '011203' AND ODR.TRAINNO='5461')")
            sqlStr.AppendLine("       )")
        Else
            sqlStr.AppendLine("   AND ODR.OFFICECODE      = @OFFICECODE")
        End If
            ...
            Using sqlDr As SqlDataReader = sqlCmd.ExecuteReader()
                If sqlDr.HasRows Then
                    ...
                    While sqlDr.Read
                        ...
                        If retVal.SuggestList(targetDate).DayInfo.IsPastDay AndAlso isGetAlldays = False Then
                            Continue While
                        End If

                        '画面表示外のデータも設定しない
                        If retVal.SuggestList(targetDate).DayInfo.IsDispArea = False AndAlso isOtGetAll = False Then
                            Continue While
                        End If
    ...
    End Function
    ''' <summary>
    ''' 空回日報情報を取得、提案表の車数を更新(受入日別日の情報を取得）
    ''' </summary>
    ''' <param name="sqlCon"></param>
    ''' <param name="dispData"></param>
    ''' <returns></returns>
    Private Function EditEmptyTurnCarsNumExtra(sqlCon As SqlConnection, dispData As DispDataClass, Optional isGetAlldays As Boolean = False) As DispDataClass
        'これもしかすると在庫テーブルから先週の払出量かと
        'オーダーは受入数な気がする(Consigneeしかないので)
        Dim sqlStr As New StringBuilder
        Dim retVal = dispData
        ...
        sqlStr.AppendLine("     , ODR.TRAINNO")
        sqlStr.AppendLine("     , format(ODR.LODDATE,'yyyy/MM/dd') AS TARGETDATE")
        sqlStr.AppendLine("     , format(ISNULL(DTL.ACTUALACCDATE,ODR.ACCDATE),'yyyy/MM/dd') AS ACCDATE")
        sqlStr.AppendLine("     , SUM(isnull(DTL.CARSNUMBER,0))    AS CARSNUMBER")
        sqlStr.AppendLine("  FROM      OIL.OIT0002_ORDER  ODR")
        sqlStr.AppendLine(" INNER JOIN OIL.OIT0003_DETAIL DTL")
        sqlStr.AppendLine("    ON ODR.ORDERNO =  DTL.ORDERNO")
        ...
        sqlStr.AppendLine("   AND ODR.DELFLG          = @DELFLG")
        sqlStr.AppendLine("   AND ODR.ORDERSTATUS    <> @ORDERSTATUS_CANCEL") 'キャンセルは含めない
        sqlStr.AppendLine(" GROUP BY DTL.OILCODE,ODR.TRAINNO,ODR.LODDATE,format(ISNULL(DTL.ACTUALACCDATE,ODR.ACCDATE),'yyyy/MM/dd')")
        Using sqlCmd As New SqlCommand(sqlStr.ToString, sqlCon)
            With sqlCmd.Parameters
                .Add("@DELFLG", SqlDbType.NVarChar).Value = C_DELETE_FLG.ALIVE
                .Add("@DATE_FROM", SqlDbType.Date).Value = dateFrom
            ...
            Using sqlDr As SqlDataReader = sqlCmd.ExecuteReader()
                If sqlDr.HasRows Then
                    ...
                    While sqlDr.Read
                        ...
                        'AccDayの設定が画面設定と同じなら設定しない
                        If retVal.SuggestList(targetDate).SuggestOrderItem(trainNo).AccAddDays = Convert.ToString(accDays) Then
                            'Continue While
                        End If
                        With retVal.SuggestList(targetDate).SuggestOrderItem(trainNo).SuggestValuesItem(oilCode)
                            If .ExtraItemValue Is Nothing Then
                                .ExtraItemValue = New Dictionary(Of String, Decimal)
                            End If
                            suggestVal = CDec(sqlDr("CARSNUMBER"))
                            .ExtraItemValue.Add(accDate, suggestVal)
                        End With

                    End While
                    ''在庫テーブルの車数計 < 受注、異受入日車数の場合は在庫設定優先のため保持しない
                    'For Each sugDateItem In retVal.SuggestList.Values
                    '    For Each sugTrItem In sugDateItem.SuggestOrderItem.Values
                    '        For Each sugOilItm In sugTrItem.SuggestValuesItem.Values
                    '            If Not IsNumeric(sugOilItm.ItemValue) Then
                    '                Continue For
                    '            End If
                    '            If sugOilItm.ExtraItemValue Is Nothing OrElse sugOilItm.ExtraItemValue.Count = 0 Then
                    '                Continue For
                    '            End If
                    '            Dim extraItemValSum = (From itm In sugOilItm.ExtraItemValue.Values Select itm).Sum
                    '            '画面の数量が多い場合は計算が狂うので保持しない
                    '            If CDec(sugOilItm.ItemValue) < extraItemValSum Then
                    '                sugOilItm.ExtraItemValue = Nothing
                    '            End If
    ...
    Private Function EditOtEmptyTurnCarsNum(sqlCon As SqlConnection, dispData As DispDataClass) As DispDataClass
        ...
        sqlStr.AppendLine("SELECT DTL.OILCODE")
        sqlStr.AppendLine("     , format(ISNULL(DTL.ACTUALACCDATE,ODR.ACCDATE),'yyyy/MM/dd') AS TARGETDATE")
        sqlStr.AppendLine("     , SUM(isnull(DTL.CARSNUMBER,0))    AS CARSNUMBER")
        sqlStr.AppendLine("  FROM      OIL.OIT0002_ORDER  ODR")
        sqlStr.AppendLine(" INNER JOIN OIL.OIT0003_DETAIL DTL")
        sqlStr.AppendLine("    ON ODR.ORDERNO =  DTL.ORDERNO")
        sqlStr.AppendLine("   AND DTL.DELFLG  =  @DELFLG")
        sqlStr.AppendLine("   AND DTL.OILCODE is not null")
        sqlStr.AppendLine(" WHERE format(ISNULL(DTL.ACTUALACCDATE,ODR.ACCDATE),'yyyy/MM/dd')   BETWEEN @DATE_FROM AND @ADATE_TO")
        sqlStr.AppendLine("   AND ODR.OFFICECODE      = @OFFICECODE")
        'sqlStr.AppendLine("   AND ODR.SHIPPERSCODE    = @SHIPPERSCODE")
        '荷主取得条件(JOINTコード考慮)↓
        sqlStr.AppendLine("   AND ((     DTL.SHIPPERSCODE   = @SHIPPERSCODE")
        ...
        sqlStr.AppendLine("   AND ODR.DELFLG          = @DELFLG")
        sqlStr.AppendLine("   AND ODR.ORDERSTATUS    <> @ORDERSTATUS_CANCEL") 'キャンセルは含めない
        sqlStr.AppendLine(" GROUP BY DTL.OILCODE,format(ISNULL(DTL.ACTUALACCDATE,ODR.ACCDATE),'yyyy/MM/dd')")
        Using sqlCmd As New SqlCommand(sqlStr.ToString, sqlCon)
            With sqlCmd.Parameters
                .Add("@DELFLG", SqlDbType.NVarChar).Value = C_DELETE_FLG.ALIVE
                .Add("@DATE_FROM", SqlDbType.Date).Value = dateFrom
        ...
        Using sqlCmd As New SqlCommand(sqlStat.ToString, sqlCon, sqlTran)
            ...
            For Each stockItem In dispDataClass.StockList.Values
                ...
                For Each daysValue In stockItem.StockItemList.Values
                    ...
                    '    Continue For
                    'End If
                    '過去日も保存対象とする(画面範囲表示での過去日※未来は画面範囲外でも受入日で跳ねる影響がある為対応)
                    If daysValue.DaysItem.IsPastDay AndAlso daysValue.DaysItem.IsDispArea = False Then
                        Continue For
                    End If
    ...
    Private Function GetThisScreenData(frvSuggestObj As FormView, repStockObj As Repeater) As DispDataClass
        ...
        ElseIf retVal.IsOtTrainMode AndAlso retVal.SuggestList IsNot Nothing AndAlso retVal.SuggestList.Count > 0 Then
            SetDispSuggestItemValue(retVal, repValArea)
        End If
    ...
    Public Class OilItem
        ...
        ''' </summary>
        ''' <returns></returns>
        Public Property OrderFromDate As String
        ''' <summary>
        ''' 受注登録可能期間TO
        ''' </summary>
        ''' <returns></returns>
    ...
    Public Class TrainListItem
        ...
        ''' <summary>
        ''' 画面表示の列車番号(現状は三重塩浜の5461を画面上は富士石油と表示するため利用)
        ''' </summary>
        ''' <returns></returns>
        Public Property DispTrainNo As String = ""
        ''' <summary>
        ''' 列車名
        ''' </summary>
        ''' <returns></returns>
    ...
    Public Class DispDataClass
        ...
        ''' <param name="onlyKawasaki">川崎のみ取得フラグ(True:川崎のみ取得,False(初期):すべて取得</param>
        ''' <returns></returns>
        Public Function GetSummarySuggestValue(dateKey As String, oilCode As String, Optional onlyKawasaki As Boolean = False) As Decimal
            ...
            For Each daysItems In Me.SuggestList.Values
                ...
                For Each tgtItm In daysItems.SuggestOrderItem.Values
                    ...
                    '三重塩浜の5461列車の車数は含めない
                    If tgtItm.TrainInfo.TrainNo = "012402" AndAlso Me.SalesOffice = "5461" Then
                        Continue For
                    End If
                    If onlyKawasaki AndAlso tgtItm.TrainInfo.TrainNo <> "川崎" Then
                        Continue For
                    End If
                    'チェックをしている値のみ合計する
                    If tgtItm.CheckValue = True OrElse tgtItm.CheckValue = False Then '20200529 チェック未チェックでもOK戻しやすいよう両条件入れておく本来このIF不要
                        ...
                        If tgtItm.SuggestValuesItem.ContainsKey(oilCode) Then
                            ...
                            Dim trainNum As Decimal = 0
                            Dim extraNumAll As Decimal = 0
                            Dim extraNumMinus As Decimal = 0
                            Dim extraNumPlus As Decimal = 0
                            Dim isTargetItemFound As Boolean = False
                            If suggestItm.ExtraItemValue IsNot Nothing Then
                                extraNumAll = (From itm In suggestItm.ExtraItemValue.Values).Sum
                                With (From itm In suggestItm.ExtraItemValue Where itm.Key <> dateKey Select itm.Value)
                                    If .Any Then
                                        extraNumMinus = .Sum
                                    End If
                                End With
                                With (From itm In suggestItm.ExtraItemValue Where itm.Key = dateKey Select itm.Value)
                                    If .Any AndAlso daysItems.DayInfo.KeyString <> targetDay Then
                            ...
                            If daysItems.DayInfo.KeyString = targetDay Then
                                isTargetItemFound = True
                                trainNum = Decimal.Parse(suggestItm.ItemValue)
                                If Not trainNum < extraNumAll Then
                                    trainNum = trainNum - extraNumMinus
                                End If
                            Else
                                trainNum = Decimal.Parse(suggestItm.ItemValue)
                                If (Not trainNum < extraNumAll) AndAlso extraNumPlus > 0 Then
                                    trainNum = extraNumPlus
                                End If
                            End If

                            If isTargetItemFound = False Then
                                Continue For
                            End If

                            If suggestItm.OilInfo.Weight <> 0 Then
                                calcVal = Math.Floor(trainNum * 45 / suggestItm.OilInfo.Weight)
                            End If
        ...
        Public Class SuggestItem
            ...
            Public Class SuggestValue
                ...
                ''' <summary>
                ''' 拡張の受入日個別設定した車数（上記ItemValueの内）となる(key受入日,車数)
                ''' </summary>
                Public ExtraItemValue As Dictionary(Of String, Decimal)
                ''' <summary>
                ''' 変更フラグ(色変用)
                ''' </summary>
                ''' <returns></returns>
    ...
    Public Class OrderItem
        ...
        Public Sub New(orderNo As String, dispDataClass As DispDataClass, chkItm As DispDataClass.SelectedSuggestValItem, procDtm As Date, userID As String, termId As String)
            ...
            If dispDataClass.SalesOffice = "012402" AndAlso dispDataClass.Consignee = "40" AndAlso chkItm.trainInfo.TrainNo = "5461" Then
                Me.OfficeCode = "011203"
                Me.OfficeName = "袖ヶ浦営業所"
            Else
