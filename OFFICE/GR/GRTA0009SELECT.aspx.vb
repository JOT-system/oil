...
Public Class GRTA0009SELECT
    ...
    Protected Sub WF_ButtonDO_Click()
        ...
        work.WF_SEL_SORG.Text = WF_SORG.Text
    ...
    Protected Sub WF_ButtonSel_Click()
        ...
        Select Case WF_FIELD.Value
            ...
            Case "WF_SORG"
                '副乗員含む 
                WF_SORG_TEXT.Text = values(1)
                WF_SORG.Text = values(0)
                WF_SORG.Focus()
        End Select
    ...
    Protected Sub SetMapValue()
        ...
        '■■■ 選択画面の入力初期値設定 ■■■
        If IsNothing(Master.MAPID) Then Master.MAPID = GRTA0009WRKINC.MAPIDS
        'メニューからの画面遷移
        If Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.MENU Then                                                   'メニューからの画面遷移
            work.Initialize()
            '○画面項目設定（変数より）処理
            SetInitialValue()
        ElseIf Context.Handler.ToString().ToUpper = C_PREV_MAP_LIST.TA0009 Then                                                   '実行画面からの画面遷移
            '■■■ 実行画面からの画面遷移 ■■■
        ...
        End If
        '○RightBox情報設定
        rightview.MAPID = GRTA0009WRKINC.MAPID
        rightview.MAPIDS = GRTA0009WRKINC.MAPIDS
    ...
    Protected Sub SetInitialValue()
        ...
        '作業部署
        Master.getFirstValue(work.WF_SEL_CAMPCODE.Text, "SORG", WF_SORG.Text)
    End Sub
    ...
    Protected Sub SetNameValue()
        ...
        '作業部署
        CodeToName("SORG", WF_SORG.Text, WF_SORG_TEXT.Text, WW_DUMMY)
    ...
    Protected Sub CodeToName(ByVal I_FIELD As String, ByVal I_VALUE As String, ByRef O_TEXT As String, ByRef O_RTN As String)
        ...
        If Not String.IsNullOrEmpty(I_VALUE) Then
            Select Case I_FIELD
                ...
                Case "SORG"
                    leftview.CodeToName(GRIS0005LeftBox.LIST_BOX_CLASSIFICATION.LC_ORG, I_VALUE, O_TEXT, O_RTN, work.CreateSORGParam(WF_CAMPCODE.Text, C_PERMISSION.REFERLANCE))                     '職務区分
                Case Else
                    O_TEXT = String.Empty
            End Select
        End If
    ...
    Protected Sub CheckParameters(ByRef O_RTN As String)
        ...
        '機能選択 WF_SORG.Text
        Master.eraseCharToIgnore(WF_SORG.Text)
        ...
        '●機能選択 WF_SORG.Text
        '単項目チェック
        WW_CHECK = WF_SORG.Text
        Master.checkFIeld(WF_CAMPCODE.Text, "SORG", WW_CHECK, WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
        If isNormal(WW_CS0024FCHECKERR) Then
            '存在チェック(LeftBoxチェック)
            If Not String.IsNullOrEmpty(WF_SORG.Text) Then
                leftview.CodeToName(GRIS0005LeftBox.LIST_BOX_CLASSIFICATION.LC_ORG, WF_SORG.Text, WF_SORG_TEXT.Text, O_RTN, work.CreateSORGParam(WF_CAMPCODE.Text, C_PERMISSION.REFERLANCE))
                If Not isNormal(O_RTN) Then
                    Master.output(C_MESSAGE_NO.INVALID_SELECTION_DATA, C_MESSAGE_TYPE.ERR)
                    WF_SORG.Focus()
                    O_RTN = C_MESSAGE_NO.INVALID_SELECTION_DATA
