...
Public Class GRMB0003SELECT
    ...
    Protected Sub Initialize()
        ...
        '○ 画面ID設定
        Master.MAPID = GRMB0003WRKINC.MAPIDS
    ...
    Protected Sub WW_MAPValueSet()
        ...
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.MENU Then             'メニューからの画面遷移
            ...
            '年月
            Master.getFirstValue(work.WF_SEL_CAMPCODE.Text, "YYMM", WF_YYMM.Text)
            Dim WW_DATE As Date
            Try
                Date.TryParse(WF_YYMM.Text, WW_DATE)
                WF_YYMM.Text = WW_DATE.ToString("yyyy/MM")
            Catch ex As Exception
                WF_YYMM.Text = Date.Now.ToString("yyyy/MM")
            End Try
        ...
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.MB0003 Then       '実行画面からの遷移
            '画面項目設定処理
            WF_CAMPCODE.Text = work.WF_SEL_CAMPCODE.Text        '会社コード
            WF_YYMM.Text = work.WF_SEL_YYMM.Text                '年月
            WF_SORG.Text = work.WF_SEL_SORG.Text                '作業部署
            WF_STAFFKBN.Text = work.WF_SEL_STAFFKBN.Text        '職務区分
        End If

        '○ RightBox情報設定
        rightview.MAPIDS = GRMB0003WRKINC.MAPIDS
        rightview.MAPID = GRMB0003WRKINC.MAPID
    ...
    Protected Sub WF_ButtonDO_Click()
        ...
        Master.eraseCharToIgnore(WF_YYMM.Text)              '年月
        ...
        work.WF_SEL_YYMM.Text = WF_YYMM.Text                '年月
    ...
    Protected Sub WW_Check(ByRef O_RTN As String)
        ...
        '年月
        Master.checkFIeld(WF_CAMPCODE.Text, "YYMM", WF_YYMM.Text, WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
        If isNormal(WW_CS0024FCHECKERR) Then
            If WF_YYMM.Text <> "" Then
                Dim WW_DATE As Date
                Try
                    Date.TryParse(WF_YYMM.Text, WW_DATE)
                    WF_YYMM.Text = WW_DATE.ToString("yyyy/MM")
                Catch ex As Exception
                    Master.output(C_MESSAGE_NO.DATE_FORMAT_ERROR, C_MESSAGE_TYPE.ABORT, "年月 : " & WF_YYMM.Text)
                    WF_YYMM.Focus()
                    O_RTN = "ERR"
                    Exit Sub
                End Try
            End If
        Else
            Master.output(WW_CS0024FCHECKERR, C_MESSAGE_TYPE.ERR, "年月 : " & WF_YYMM.Text)
            WF_YYMM.Focus()
            O_RTN = "ERR"
            Exit Sub
        End If
            ...
            With leftview
                Select Case WF_LeftMViewChange.Value
                    Case LIST_BOX_CLASSIFICATION.LC_CALENDAR
                        ...
                        .WF_Calendar.Text = WF_YYMM.Text & "/01"        '年月
                        .activeCalendar()
