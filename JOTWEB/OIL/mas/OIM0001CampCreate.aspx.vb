...
''' </summary>
''' <remarks></remarks>
Public Class OIM0001UserCreate
    Inherits Page
    ...
    Protected Sub WW_MAPValueSet()
        ...
        '会社名称
        WF_NAME.Text = work.WF_SEL_NAME.Text

        '会社名称（短）
        WF_NAMES.Text = work.WF_SEL_NAMES.Text

        '会社カナ名称
        WF_NAMEKANA.Text = work.WF_SEL_NAMEKANA.Text

        '会社カナ名称（短）
        WF_NAMEKANAS.Text = work.WF_SEL_NAMEKANAS.Text
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                ...
                For Each OIM0001row As DataRow In OIM0001tbl.Rows
                    ...
                    ''名称取得
                    'CODENAME_get("CAMPCODE", OIM0001row("CAMPCODE"), OIM0001row("CAMPNAMES"), WW_DUMMY)                               '会社コード
                    'CODENAME_get("ORG", OIM0001row("ORG"), OIM0001row("ORGNAMES"), WW_DUMMY)                                          '組織コード
                Next
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0001C SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0001C Select"
    ...
    Protected Sub DetailBoxToOIM0001INPtbl(ByRef O_RTN As String)
        ...
        OIM0001INProw("UPDTIMSTP") = 0
        OIM0001INProw("SELECT") = 1
        OIM0001INProw("HIDDEN") = 0

        OIM0001INProw("DELFLG") = WF_DELFLG.Text                     '削除フラグ

        OIM0001INProw("USERID") = WF_USERID.Text              'ユーザID

        OIM0001INProw("STAFFNAMES") = WF_STAFFNAMES.Text              '社員名（短）

        OIM0001INProw("STAFFNAMEL") = WF_STAFFNAMEL.Text              '社員名（長）

        OIM0001INProw("MAPID") = WF_MAPID.Text              '画面ＩＤ

        OIM0001INProw("PASSWORD") = WF_PASSWORD.Text              'パスワード

        OIM0001INProw("MISSCNT") = WF_MISSCNT.Text              '誤り回数

        OIM0001INProw("PASSENDYMD") = WF_PASSENDYMD.Text              'パスワード有効期限

        OIM0001INProw("STYMD") = WF_STYMD.Text              '開始年月日

        OIM0001INProw("ENDYMD") = WF_ENDYMD.Text              '終了年月日

        OIM0001INProw("CAMPCODE") = WF_CAMPCODE.Text              '会社コード

        OIM0001INProw("ORG") = WF_ORG.Text              '組織コード

        OIM0001INProw("EMAIL") = WF_EMAIL.Text              'メールアドレス

        OIM0001INProw("MENUROLE") = WF_MENUROLE.Text              'メニュー表示制御ロール

        OIM0001INProw("MAPROLE") = WF_MAPROLE.Text              '画面参照更新制御ロール

        OIM0001INProw("VIEWPROFID") = WF_VIEWPROFID.Text              '画面表示項目制御ロール

        OIM0001INProw("RPRTPROFID") = WF_RPRTPROFID.Text              'エクセル出力制御ロール

        OIM0001INProw("VARIANT") = WF_VARIANT.Text              '画面初期値ロール

        OIM0001INProw("APPROVALID") = WF_APPROVALID.Text              '承認権限ロール
    ...
    Protected Sub INPTableCheck(ByRef O_RTN As String)
        ...
        For Each OIM0001INProw As DataRow In OIM0001INPtbl.Rows
            ...
            '削除フラグ(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "DELFLG", OIM0001INProw("DELFLG"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            ...
            'ユーザID(バリデーションチェック)
            Master.CheckField(Master.USERCAMP, "USERID", OIM0001INProw("USERID"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'CODENAME_get("USERID", OIM0001INProw("USERID"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            '社員名（短）(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "STAFFNAMES", OIM0001INProw("STAFFNAMES"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'CODENAME_get("STAFFNAMES", OIM0001INProw("STAFFNAMES"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            '社員名（長）(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "STAFFNAMEL", OIM0001INProw("STAFFNAMEL"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'CODENAME_get("STAFFNAMEL", OIM0001INProw("STAFFNAMEL"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            '誤り回数(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "MISSCNT", OIM0001INProw("MISSCNT"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'If OIM0001INProw("MISSCNT") <> "" Then
                'CODENAME_get("MISSCNT", OIM0001INProw("MISSCNT"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            'パスワード(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "PASSWORD", OIM0001INProw("PASSWORD"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'CODENAME_get("PASSWORD", OIM0001INProw("PASSWORD"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "PASSENDYMD", OIM0001INProw("PASSENDYMD"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '年月日チェック
                WW_CheckDate(OIM0001INProw("PASSENDYMD"), "パスワード有効期限", WW_CS0024FCHECKERR, dateErrFlag)
                If dateErrFlag = "1" Then
                    ...
                    Exit Sub
                Else
                    OIM0001INProw("PASSENDYMD") = CDate(OIM0001INProw("PASSENDYMD")).ToString("yyyy/MM/dd")
                End If
            Else
                WW_CheckMES1 = "・更新できないレコード(パスワード有効期限エラー)です。"
                WW_CheckMES2 = WW_CS0024FCHECKREPORT
            ...
            '開始年月日(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "STYMD", OIM0001INProw("STYMD"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '年月日チェック
            ...
            '終了年月日(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "ENDYMD", OIM0001INProw("ENDYMD"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '年月日チェック
            ...
            '組織コード(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "ORG", OIM0001INProw("ORG"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("ORG", OIM0001INProw("ORG"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            'メールアドレス(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "EMAIL", OIM0001INProw("EMAIL"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'CODENAME_get("EMAIL", OIM0001INProw("EMAIL"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "MENUROLE", OIM0001INProw("MENUROLE"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("MENU", OIM0001INProw("MENUROLE"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "MAPROLE", OIM0001INProw("MAPROLE"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("MAP", OIM0001INProw("MAPROLE"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "VIEWPROFID", OIM0001INProw("VIEWPROFID"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("VIEW", OIM0001INProw("VIEWPROFID"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "RPRTPROFID", OIM0001INProw("RPRTPROFID"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("XML", OIM0001INProw("RPRTPROFID"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            Master.CheckField(Master.USERCAMP, "VARIANT", OIM0001INProw("VARIANT"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                ''値存在チェック
                'If OIM0001INProw("VARIANT") <> "" Then
                'CODENAME_get("VARIANT", OIM0001INProw("VARIANT"), WW_DUMMY, WW_RTN_SW)
                'If Not isNormal(WW_RTN_SW) Then
            ...
            '承認権限ロール(バリデーションチェック）
            Master.CheckField(Master.USERCAMP, "APPROVALID", OIM0001INProw("APPROVALID"), WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If isNormal(WW_CS0024FCHECKERR) Then
                '値存在チェック
                CODENAME_get("APPROVAL", OIM0001INProw("APPROVALID"), WW_DUMMY, WW_RTN_SW)
                If Not isNormal(WW_RTN_SW) Then
            ...
            If OIM0001INProw("USERID") = work.WF_SEL_USERID.Text Then
    ...
    Protected Sub WW_CheckERR(ByVal MESSAGE1 As String, ByVal MESSAGE2 As String, Optional ByVal OIM0001row As DataRow = Nothing)
        ...
        If Not IsNothing(OIM0001row) Then
            WW_ERR_MES &= ControlChars.NewLine & "  --> ユーザID =" & OIM0001row("USERID") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 社員名（短） =" & OIM0001row("STAFFNAMES") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 社員名（長） =" & OIM0001row("STAFFNAMEL") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 画面ＩＤ =" & OIM0001row("MAPID") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> パスワード =" & OIM0001row("PASSWORD") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 誤り回数 =" & OIM0001row("MISSCNT") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> パスワード有効期限 =" & OIM0001row("PASSENDYMD") & " , "
            ...
            WW_ERR_MES &= ControlChars.NewLine & "  --> 組織コード =" & OIM0001row("ORG") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> メールアドレス =" & OIM0001row("EMAIL") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> メニュー表示制御ロール =" & OIM0001row("MENUROLE") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 画面参照更新制御ロール =" & OIM0001row("MAPROLE") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 画面表示項目制御ロール =" & OIM0001row("VIEWPROFID") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> エクセル出力制御ロール =" & OIM0001row("RPRTPROFID") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 画面初期値ロール =" & OIM0001row("VARIANT") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 承認権限ロール =" & OIM0001row("APPROVALID") & " , "
            WW_ERR_MES &= ControlChars.NewLine & "  --> 削除フラグ =" & OIM0001row("DELFLG")
        End If
    ...
    Protected Sub OIM0001tbl_UPD()
        ...
        For Each OIM0001INProw As DataRow In OIM0001INPtbl.Rows
            ...
            'KEY項目が等しい時
            For Each OIM0001row As DataRow In OIM0001tbl.Rows
                If OIM0001row("USERID") = OIM0001INProw("USERID") AndAlso
                    OIM0001row("STYMD") = OIM0001INProw("STYMD") Then
                    'KEY項目以外の項目に変更がないときは「操作」の項目は空白にする
                    If OIM0001row("DELFLG") = OIM0001INProw("DELFLG") AndAlso
                        OIM0001row("STAFFNAMES") = OIM0001INProw("STAFFNAMES") AndAlso
                        OIM0001row("STAFFNAMEL") = OIM0001INProw("STAFFNAMEL") AndAlso
                        OIM0001row("MAPID") = OIM0001INProw("MAPID") AndAlso
                        OIM0001row("PASSWORD") = OIM0001INProw("PASSWORD") AndAlso
                        OIM0001row("MISSCNT") = OIM0001INProw("MISSCNT") AndAlso
                        OIM0001row("PASSENDYMD") = OIM0001INProw("PASSENDYMD") AndAlso
                        OIM0001row("ENDYMD") = OIM0001INProw("ENDYMD") AndAlso
                        OIM0001row("CAMPCODE") = OIM0001INProw("CAMPCODE") AndAlso
                        OIM0001row("ORG") = OIM0001INProw("ORG") AndAlso
                        OIM0001row("EMAIL") = OIM0001INProw("EMAIL") AndAlso
                        OIM0001row("MENUROLE") = OIM0001INProw("MENUROLE") AndAlso
                        OIM0001row("MAPROLE") = OIM0001INProw("MAPROLE") AndAlso
                        OIM0001row("VIEWPROFID") = OIM0001INProw("VIEWPROFID") AndAlso
                        OIM0001row("RPRTPROFID") = OIM0001INProw("RPRTPROFID") AndAlso
                        OIM0001row("VARIANT") = OIM0001INProw("VARIANT") AndAlso
                        OIM0001row("APPROVALID") = OIM0001INProw("APPROVALID") AndAlso
    ...
    Protected Sub TBL_UPDATE_SUB(ByRef OIM0001INProw As DataRow)
        ...
        For Each OIM0001row As DataRow In OIM0001tbl.Rows
            ...
            '同一レコードか判定
            If OIM0001INProw("USERID") = OIM0001row("USERID") AndAlso
                OIM0001INProw("STYMD") = OIM0001row("STYMD") Then
                '画面入力テーブル項目設定
                OIM0001INProw("LINECNT") = OIM0001row("LINECNT")
                OIM0001INProw("OPERATION") = C_LIST_OPERATION_CODE.UPDATING
                OIM0001INProw("UPDTIMSTP") = OIM0001row("UPDTIMSTP")
                OIM0001INProw("SELECT") = 1
                OIM0001INProw("HIDDEN") = 0
