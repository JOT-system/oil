...
Public Class OIM0021LoadReserveList
        ...
        Try
            If IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    '○ 画面表示データ復元
                    Master.RecoverTable(OIM0021tbl)
    ...
    Protected Sub Initialize()
        ...
        '○画面ID設定
        Master.MAPID = OIM0021WRKINC.MAPIDL
        '○HELP表示有無設定
    ...
    Protected Sub WW_MAPValueSet()
        ...
        '○ 検索画面からの遷移
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0021S Then
            'Grid情報保存先のファイル名
            Master.CreateXMLSaveFile()
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0021C Then
    ...
    Protected Sub GridViewInitialize()
        ...
        If Context.Handler.ToString().ToUpper() <> C_PREV_MAP_LIST.OIM0021C Then
            '○ 画面表示データ取得
        ...
        '○ 画面表示データ保存
        Master.SaveTable(OIM0021tbl)

        '〇 一覧の件数を取得
        Me.WF_ListCNT.Text = "件数：" + OIM0021tbl.Rows.Count.ToString()

        '○ 一覧表示データ編集(性能対策)
        Dim TBLview As DataView = New DataView(OIM0021tbl)
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As SqlConnection)
        ...
        If IsNothing(OIM0021tbl) Then
            OIM0021tbl = New DataTable
        End If

        If OIM0021tbl.Columns.Count <> 0 Then
            OIM0021tbl.Columns.Clear()
        End If
        ...
        Dim SQLStr As String =
            ...
            & "   0                                                            AS LINECNT " _
            & " , ''                                                           AS OPERATION " _
            & " , CAST(OIM0021.UPDTIMSTP AS bigint)                            AS UPDTIMSTP " _
            & " , 1                                                            AS 'SELECT' " _
            & " , 0                                                            AS HIDDEN " _
        ...
        If Not String.IsNullOrEmpty(work.WF_SEL_OILCODE.Text) Then
            SQLStr &= "    AND OIM0021.OILCODE        = @P6"
        End If

        '油種細分コード
        If Not String.IsNullOrEmpty(work.WF_SEL_SEGMENTOILCODE.Text) Then
            SQLStr &= "    AND OIM0021.SEGMENTOILCODE = @P7"
        End If

        SQLStr &=
              " ORDER BY" _
            & "    RIGHT('000000' + CAST(OIM0021.OFFICECODE AS NVARCHAR), 6)" _
            & "    , OIM0021.FROMYMD" _
            & "    , OIM0021.LOAD" _
            & "    , RIGHT('0000000000' + CAST(OIM0021.OILCODE AS NVARCHAR), 10)" _
            & "    , RIGHT('0' + CAST(OIM0021.SEGMENTOILCODE AS NVARCHAR), 1)"
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                ...
                If Not String.IsNullOrEmpty(work.WF_SEL_FROMYMD.Text) Then
                    ...
                    PARA3.Value = work.WF_SEL_FROMYMD.Text
                End If
                ...
                If Not String.IsNullOrEmpty(work.WF_SEL_OILCODE.Text) Then
                    ...
                    PARA6.Value = work.WF_SEL_OILCODE.Text
                End If

                '油種細分コード
                If Not String.IsNullOrEmpty(work.WF_SEL_SEGMENTOILCODE.Text) Then
                    Dim PARA7 As SqlParameter = SQLcmd.Parameters.Add("@P7", SqlDbType.NVarChar, 1)
                    PARA7.Value = work.WF_SEL_SEGMENTOILCODE.Text
                End If

                Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                    '○ フィールド名とフィールドの型を取得
                    For index As Integer = 0 To SQLdr.FieldCount - 1
                        OIM0021tbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                    Next

                    '○ テーブル検索結果をテーブル格納
                    OIM0021tbl.Load(SQLdr)
                End Using
        ...
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0021L SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0021L Select"
    ...
    Protected Sub DisplayGrid()
        ...
        '○ 表示対象行カウント(絞り込み対象)
        For Each OIM0021row As DataRow In OIM0021tbl.Rows
            If OIM0021row("HIDDEN") = 0 Then
                WW_DataCNT += 1
                '行(LINECNT)を再設定する。既存項目(SELECT)を利用
                OIM0021row("SELECT") = WW_DataCNT
            End If
        Next
    ...
    Protected Sub WF_ButtonINSERT_Click()
        ...
        '適用終了年月日
        work.WF_SEL_TOYMD2.Text = ""

        '荷重
        work.WF_SEL_LOAD2.Text = ""
        ...
        '予約数量
        work.WF_SEL_RESERVEDQUANTITY.Text = ""
    ...
    Protected Sub UpdateMaster(ByVal SQLcon As SqlConnection)
        ...
        Dim SQLStr As String =
            ...
            & "    AND LOAD           = @P05 " _
            & "    AND OILCODE        = @P06 " _
            & "    AND SEGMENTOILCODE = @P07 ;" _
            & " OPEN hensuu ;" _
            & " FETCH NEXT FROM hensuu INTO @hensuu ;" _
            & " IF (@@FETCH_STATUS = 0)" _
            & "    UPDATE OIL.OIM0021_LOADRESERVE" _
            & "    SET" _
            & "        DELFLG             = @P00" _
            & "        , TOYMD            = @P03" _
            & "        , MODEL            = @P04" _
            & "        , RESERVEDQUANTITY = @P08" _
            & "        , UPDYMD           = @P12" _
            & "        , UPDUSER          = @P13" _
            & "        , UPDTERMID        = @P14" _
            & "        , RECEIVEYMD       = @P15" _
            ...
            & "    AND SEGMENTOILCODE     = @P07 ;" _
            & " IF (@@FETCH_STATUS <> 0)" _
            & "    INSERT INTO OIL.OIM0021_LOADRESERVE" _
            & "        (DELFLG" _
            & "        , OFFICECODE" _
            & "        , FROMYMD" _
            & "        , TOYMD" _
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon), SQLcmdJnl As New SqlCommand(SQLJnl, SQLcon)
                ...
                Dim PARA02 As SqlParameter = SQLcmd.Parameters.Add("@P02", SqlDbType.Date)                  '適用開始年月日
                Dim PARA03 As SqlParameter = SQLcmd.Parameters.Add("@P03", SqlDbType.Date)                  '適用終了年月日
                Dim PARA04 As SqlParameter = SQLcmd.Parameters.Add("@P04", SqlDbType.NVarChar, 20)          '型式
                Dim PARA05 As SqlParameter = SQLcmd.Parameters.Add("@P05", SqlDbType.Float)                 '荷重
                Dim PARA06 As SqlParameter = SQLcmd.Parameters.Add("@P06", SqlDbType.NVarChar, 10)          '油種コード
                Dim PARA07 As SqlParameter = SQLcmd.Parameters.Add("@P07", SqlDbType.NVarChar, 1)           '油種細分コード
                Dim PARA08 As SqlParameter = SQLcmd.Parameters.Add("@P08", SqlDbType.Float)                 '予約数量
                ...
                Dim JPARA02 As SqlParameter = SQLcmdJnl.Parameters.Add("@P02", SqlDbType.Date)              '適用開始年月日
                Dim JPARA05 As SqlParameter = SQLcmdJnl.Parameters.Add("@P05", SqlDbType.Float)             '荷重
                Dim JPARA06 As SqlParameter = SQLcmdJnl.Parameters.Add("@P06", SqlDbType.NVarChar, 10)      '油種コード
                Dim JPARA07 As SqlParameter = SQLcmdJnl.Parameters.Add("@P07", SqlDbType.NVarChar, 1)       '油種細分コード

                For Each OIM0021row As DataRow In OIM0021tbl.Rows
                    If Trim(OIM0021row("OPERATION")) = C_LIST_OPERATION_CODE.UPDATING OrElse
                        Trim(OIM0021row("OPERATION")) = C_LIST_OPERATION_CODE.INSERTING OrElse
                        Trim(OIM0021row("OPERATION")) = C_LIST_OPERATION_CODE.SELECTED Then
                        Dim WW_DATENOW As DateTime = Date.Now

                        'DB更新
                        PARA00.Value = OIM0021row("DELFLG")
                        PARA01.Value = OIM0021row("OFFICECODE")
                        PARA02.Value = OIM0021row("FROMYMD")
                        PARA03.Value = OIM0021row("TOYMD")
                        PARA04.Value = OIM0021row("MODEL")
                        PARA05.Value = OIM0021row("LOAD")
                        PARA06.Value = OIM0021row("OILCODE")
                        PARA07.Value = OIM0021row("SEGMENTOILCODE")
                        PARA08.Value = OIM0021row("RESERVEDQUANTITY")
                        ...
                        '更新ジャーナル出力
                        JPARA01.Value = OIM0021row("OFFICECODE")
                        JPARA02.Value = OIM0021row("FROMYMD")
                        JPARA05.Value = OIM0021row("LOAD")
                        JPARA06.Value = OIM0021row("OILCODE")
                        JPARA07.Value = OIM0021row("SEGMENTOILCODE")

                        Using SQLdr As SqlDataReader = SQLcmdJnl.ExecuteReader()
                            If IsNothing(OIM0021UPDtbl) Then
                                OIM0021UPDtbl = New DataTable

                                For index As Integer = 0 To SQLdr.FieldCount - 1
                                    OIM0021UPDtbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                                Next
                            End If

                            OIM0021UPDtbl.Clear()
                            OIM0021UPDtbl.Load(SQLdr)
                        End Using

                        For Each OIM0021UPDrow As DataRow In OIM0021UPDtbl.Rows
                            CS0020JOURNAL.TABLENM = "OIM0021L"
                            CS0020JOURNAL.ACTION = "UPDATE_INSERT"
                            CS0020JOURNAL.ROW = OIM0021UPDrow
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0021L UPDATE_INSERT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                             'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0021L UPDATE_INSERT"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                                 'ログ出力
    ...
    Protected Sub WF_ButtonDownload_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0021tbl                        'データ参照  Table
    ...
    Protected Sub WF_ButtonPrint_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0021tbl                        'データ参照Table
        CS0030REPORT.CS0030REPORT()
        If Not isNormal(CS0030REPORT.ERR) Then
            If CS0030REPORT.ERR = C_MESSAGE_NO.REPORT_EXCEL_NOT_FOUND_ERROR Then
                Master.Output(CS0030REPORT.ERR, C_MESSAGE_TYPE.ERR, needsPopUp:=True)
    ...
    Protected Sub WF_ButtonLAST_Click()
        ...
        '○ ソート
        Dim TBLview As New DataView(OIM0021tbl)
    ...
    Protected Sub WF_Grid_DBClick()
        ...
        '選択行
        work.WF_SEL_LINECNT.Text = OIM0021tbl.Rows(WW_LINECNT)("LINECNT")

        '管轄営業所
        work.WF_SEL_OFFICECODE2.Text = OIM0021tbl.Rows(WW_LINECNT)("OFFICECODE")

        '適用開始年月日
        work.WF_SEL_FROMYMD2.Text = OIM0021tbl.Rows(WW_LINECNT)("FROMYMD")

        '適用終了年月日
        work.WF_SEL_TOYMD2.Text = OIM0021tbl.Rows(WW_LINECNT)("TOYMD")

        '型式
        work.WF_SEL_MODEL.Text = OIM0021tbl.Rows(WW_LINECNT)("MODEL")

        '荷重
        work.WF_SEL_LOAD2.Text = OIM0021tbl.Rows(WW_LINECNT)("LOAD")

        '油種コード
        work.WF_SEL_OILCODE2.Text = OIM0021tbl.Rows(WW_LINECNT)("OILCODE")

        '油種細分コード
        work.WF_SEL_SEGMENTOILCODE2.Text = OIM0021tbl.Rows(WW_LINECNT)("SEGMENTOILCODE")

        '油種細分コード
        work.WF_SEL_RESERVEDQUANTITY.Text = OIM0021tbl.Rows(WW_LINECNT)("RESERVEDQUANTITY")
        ...
        '○ 選択明細の状態を設定
        Select Case OIM0021tbl.Rows(WW_LINECNT)("OPERATION")
            Case C_LIST_OPERATION_CODE.NODATA
                OIM0021tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.NODISP
                OIM0021tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.SELECTED
                OIM0021tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.UPDATING
                OIM0021tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.UPDATING
            Case C_LIST_OPERATION_CODE.ERRORED
                OIM0021tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.ERRORED
        End Select
    ...
    Protected Sub WF_FILEUPLOAD()
        ...
        '○ XLSUPLOAD明細⇒INPtbl
        Master.CreateEmptyTable(OIM0021INPtbl)
        ...
        For Each XLSTBLrow As DataRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            If WW_COLUMNS.IndexOf("OFFICECODE") >= 0 Then
                OIM0021INProw("OFFICECODE") = XLSTBLrow("OFFICECODE")
            End If

            '適用開始年月日
            If WW_COLUMNS.IndexOf("FROMYMD") >= 0 Then
                OIM0021INProw("FROMYMD") = XLSTBLrow("FROMYMD")
            End If

            '適用終了年月日
            If WW_COLUMNS.IndexOf("TOYMD") >= 0 Then
                OIM0021INProw("TOYMD") = XLSTBLrow("TOYMD")
            End If

            '型式
            If WW_COLUMNS.IndexOf("MODEL") >= 0 Then
                OIM0021INProw("MODEL") = XLSTBLrow("MODEL")
            End If

            '荷重
            If WW_COLUMNS.IndexOf("LOAD") >= 0 Then
                OIM0021INProw("LOAD") = XLSTBLrow("LOAD")
            Else
                OIM0021INProw("LOAD") = 0.0
            End If

            '油種コード
            If WW_COLUMNS.IndexOf("OILCODE") >= 0 Then
                OIM0021INProw("OILCODE") = XLSTBLrow("OILCODE")
            End If

            '油種細分コード
            If WW_COLUMNS.IndexOf("SEGMENTOILCODE") >= 0 Then
                OIM0021INProw("SEGMENTOILCODE") = XLSTBLrow("SEGMENTOILCODE")
            End If

            '予約数量
            If WW_COLUMNS.IndexOf("RESERVEDQUANTITY") >= 0 Then
                OIM0021INProw("RESERVEDQUANTITY") = XLSTBLrow("RESERVEDQUANTITY")
            Else
                OIM0021INProw("RESERVEDQUANTITY") = 0.0
            End If

            '削除フラグ
            If WW_COLUMNS.IndexOf("DELFLG") >= 0 Then
                OIM0021INProw("DELFLG") = XLSTBLrow("DELFLG")
            Else
    ...
    Protected Sub INPTableCheck(ByRef O_RTN As String)
        ...
        For Each OIM0021INProw As DataRow In OIM0021INPtbl.Rows
            ...
            Master.CheckField(work.WF_SEL_LOAD.Text, "LOAD", WW_TEXT, WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If Not isNormal(WW_CS0024FCHECKERR) Then
            ...
            Master.CheckField(work.WF_SEL_RESERVEDQUANTITY.Text, "RESERVEDQUANTITY", WW_TEXT, WW_CS0024FCHECKERR, WW_CS0024FCHECKREPORT)
            If Not isNormal(WW_CS0024FCHECKERR) Then
