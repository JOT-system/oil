...
Public Class OIM0014LoadcalcList
    Inherits Page
        ...
        Try
            If IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    '○ 画面表示データ復元
                    Master.RecoverTable(OIM0014tbl)
    ...
    Protected Sub Initialize()
        ...
        '○画面ID設定
        Master.MAPID = OIM0014WRKINC.MAPIDL
        '○HELP表示有無設定
    ...
    Protected Sub WW_MAPValueSet()
        ...
        '○ 検索画面からの遷移
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0014S Then
            'Grid情報保存先のファイル名
            Master.CreateXMLSaveFile()
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0014C Then
    ...
    Protected Sub GridViewInitialize()
        ...
        If Context.Handler.ToString().ToUpper() <> C_PREV_MAP_LIST.OIM0014C Then
            '○ 画面表示データ取得
        ...
        '○ 画面表示データ保存
        Master.SaveTable(OIM0014tbl)

        '〇 一覧の件数を取得
        Me.WF_ListCNT.Text = "件数：" + OIM0014tbl.Rows.Count.ToString()

        '○ 一覧表示データ編集(性能対策)
        Dim TBLview As DataView = New DataView(OIM0014tbl)
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As SqlConnection)
        ...
        If IsNothing(OIM0014tbl) Then
            OIM0014tbl = New DataTable
        End If

        If OIM0014tbl.Columns.Count <> 0 Then
            OIM0014tbl.Columns.Clear()
        End If
        ...
        Dim SQLStr As String =
            ...
            & "   0                                                            AS LINECNT " _
            & " , ''                                                           AS OPERATION " _
            & " , CAST(OIM0014.UPDTIMSTP AS bigint)                            AS UPDTIMSTP " _
            & " , 1                                                            AS 'SELECT' " _
            & " , 0                                                            AS HIDDEN " _
            & " , ISNULL(RTRIM(OIM0014.DELFLG), '')                            AS DELFLG " _
            & " , ISNULL(RTRIM(OIM0014.PLANTCODE), '')                         AS PLANTCODE " _
            & " , ISNULL(RTRIM(OIM0014.BIGOILCODE), '')                        AS BIGOILCODE " _
            & " , ISNULL(RTRIM(OIM0014.CHECKOILCODE), '')                      AS CHECKOILCODE " _
            & " , ISNULL(RTRIM(OIM0014.TANKCOUNT), '')                         AS TANKCOUNT " _
            & " FROM OIL.OIM0014_LOADCALC OIM0014 " _
            & " WHERE OIM0014.DELFLG <> @P1 "

        '○ 条件指定で指定されたものでSQLで可能なものを追加する
        '基地コード
        If Not String.IsNullOrEmpty(work.WF_SEL_PLANTCODE.Text) Then
            SQLStr &= "    AND OIM0014.PLANTCODE      = @P2"
        End If

        '油種大分類コード
        If Not String.IsNullOrEmpty(work.WF_SEL_BIGOILCODE.Text) Then
            SQLStr &= "    AND OIM0014.BIGOILCODE        = @P3"
        End If

        '積込チェック用油種コード
        If Not String.IsNullOrEmpty(work.WF_SEL_CHECKOILCODE.Text) Then
            SQLStr &= "    AND OIM0014.CHECKOILCODE = @P4"
        End If

        '車数
        If Not String.IsNullOrEmpty(work.WF_SEL_TANKCOUNT.Text) Then
            SQLStr &= "    AND OIM0014.TANKCOUNT     = @P5"
        End If

        SQLStr &=
              " ORDER BY" _
            & "    RIGHT('0000' + CAST(OIM0014.PLANTCODE AS NVARCHAR), 4)"
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                ...
                If Not String.IsNullOrEmpty(work.WF_SEL_BIGOILCODE.Text) Then
                    Dim PARA5 As SqlParameter = SQLcmd.Parameters.Add("@P3", SqlDbType.NVarChar, 1)     '油種大分類コード
                    PARA5.Value = work.WF_SEL_BIGOILCODE.Text
                End If

                If Not String.IsNullOrEmpty(work.WF_SEL_CHECKOILCODE.Text) Then
                    Dim PARA6 As SqlParameter = SQLcmd.Parameters.Add("@P4", SqlDbType.NVarChar, 4)     '積込チェック用油種コード
                    PARA6.Value = work.WF_SEL_CHECKOILCODE.Text
                End If

                If Not String.IsNullOrEmpty(work.WF_SEL_TANKCOUNT.Text) Then
                    Dim PARA3 As SqlParameter = SQLcmd.Parameters.Add("@P5", SqlDbType.Int)             '車数
                    PARA3.Value = work.WF_SEL_TANKCOUNT.Text
                End If

                Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                    '○ フィールド名とフィールドの型を取得
                    For index As Integer = 0 To SQLdr.FieldCount - 1
                        OIM0014tbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                    Next

                    '○ テーブル検索結果をテーブル格納
                    OIM0014tbl.Load(SQLdr)
                End Using

                Dim i As Integer = 0
                For Each OIM0014row As DataRow In OIM0014tbl.Rows
                    i += 1
                    OIM0014row("LINECNT") = i        'LINECNT
                Next
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0014L SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0014L Select"
    ...
    Protected Sub DisplayGrid()
        ...
        '○ 表示対象行カウント(絞り込み対象)
        For Each OIM0014row As DataRow In OIM0014tbl.Rows
            If OIM0014row("HIDDEN") = 0 Then
                WW_DataCNT += 1
                '行(LINECNT)を再設定する。既存項目(SELECT)を利用
                OIM0014row("SELECT") = WW_DataCNT
            End If
        Next
    ...
    Protected Sub WF_ButtonINSERT_Click()
        ...
        '積込チェック用油種コード(登録(新規追加用))
        work.WF_SEL_CHECKOILCODE2.Text = ""

        '車数(登録(新規追加用))
        work.WF_SEL_TANKCOUNT2.Text = "0"
    ...
    Protected Sub UpdateMaster(ByVal SQLcon As SqlConnection)
        ...
        Dim SQLStr As String =
            ...
            & "        CHECKOILCODE = @P03 ;" _
            & " IF (@@FETCH_STATUS <> 0)" _
            & "    INSERT INTO OIL.OIM0014_LOADCALC" _
            & "        (DELFLG" _
            & "        , PLANTCODE" _
            & "        , BIGOILCODE" _
            & "        , CHECKOILCODE" _
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon), SQLcmdJnl As New SqlCommand(SQLJnl, SQLcon)
                ...
                Dim PARA02 As SqlParameter = SQLcmd.Parameters.Add("@P02", SqlDbType.NVarChar, 1)           '油種大分類コード
                Dim PARA03 As SqlParameter = SQLcmd.Parameters.Add("@P03", SqlDbType.NVarChar, 4)           '積込チェック用油種コード
                Dim PARA04 As SqlParameter = SQLcmd.Parameters.Add("@P04", SqlDbType.Int)                   '車数
                ...
                Dim JPARA02 As SqlParameter = SQLcmdJnl.Parameters.Add("@P02", SqlDbType.NVarChar, 1)       '油種大分類コード
                Dim JPARA03 As SqlParameter = SQLcmdJnl.Parameters.Add("@P03", SqlDbType.NVarChar, 4)       '積込チェック用油種コード

                For Each OIM0014row As DataRow In OIM0014tbl.Rows
                    If Trim(OIM0014row("OPERATION")) = C_LIST_OPERATION_CODE.UPDATING OrElse
                        Trim(OIM0014row("OPERATION")) = C_LIST_OPERATION_CODE.INSERTING OrElse
                        Trim(OIM0014row("OPERATION")) = C_LIST_OPERATION_CODE.SELECTED Then
                        Dim WW_DATENOW As DateTime = Date.Now

                        'DB更新
                        PARA00.Value = OIM0014row("DELFLG")
                        PARA01.Value = OIM0014row("PLANTCODE")
                        PARA02.Value = OIM0014row("BIGOILCODE")
                        PARA03.Value = OIM0014row("CHECKOILCODE")
                        PARA04.Value = OIM0014row("TANKCOUNT")
                        ...
                        '更新ジャーナル出力
                        JPARA00.Value = OIM0014row("DELFLG")
                        JPARA01.Value = OIM0014row("PLANTCODE")
                        JPARA02.Value = OIM0014row("BIGOILCODE")
                        JPARA03.Value = OIM0014row("CHECKOILCODE")

                        Using SQLdr As SqlDataReader = SQLcmdJnl.ExecuteReader()
                            If IsNothing(OIM0014UPDtbl) Then
                                OIM0014UPDtbl = New DataTable

                                For index As Integer = 0 To SQLdr.FieldCount - 1
                                    OIM0014UPDtbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                                Next
                            End If

                            OIM0014UPDtbl.Clear()
                            OIM0014UPDtbl.Load(SQLdr)
                        End Using

                        For Each OIM0014UPDrow As DataRow In OIM0014UPDtbl.Rows
                            CS0020JOURNAL.TABLENM = "OIM0014L"
                            CS0020JOURNAL.ACTION = "UPDATE_INSERT"
                            CS0020JOURNAL.ROW = OIM0014UPDrow
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0014L UPDATE_INSERT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                             'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0014L UPDATE_INSERT"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                                 'ログ出力
    ...
    Protected Sub WF_ButtonDownload_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0014tbl                        'データ参照  Table
    ...
    Protected Sub WF_ButtonPrint_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0014tbl                        'データ参照Table
        CS0030REPORT.CS0030REPORT()
        If Not isNormal(CS0030REPORT.ERR) Then
            If CS0030REPORT.ERR = C_MESSAGE_NO.REPORT_EXCEL_NOT_FOUND_ERROR Then
                Master.Output(CS0030REPORT.ERR, C_MESSAGE_TYPE.ERR, needsPopUp:=True)
    ...
    Protected Sub WF_ButtonLAST_Click()
        ...
        '○ ソート
        Dim TBLview As New DataView(OIM0014tbl)
    ...
    Protected Sub WF_Grid_DBClick()
        ...
        '選択行
        work.WF_SEL_LINECNT.Text = OIM0014tbl.Rows(WW_LINECNT)("LINECNT")

        '基地コード
        work.WF_SEL_PLANTCODE2.Text = OIM0014tbl.Rows(WW_LINECNT)("PLANTCODE")

        '油種大分類コード
        work.WF_SEL_BIGOILCODE2.Text = OIM0014tbl.Rows(WW_LINECNT)("BIGOILCODE")

        '積込チェック用油種コード
        work.WF_SEL_CHECKOILCODE2.Text = OIM0014tbl.Rows(WW_LINECNT)("CHECKOILCODE")

        '車数
        work.WF_SEL_TANKCOUNT2.Text = OIM0014tbl.Rows(WW_LINECNT)("TANKCOUNT")

        '削除フラグ
        work.WF_SEL_DELFLG.Text = OIM0014tbl.Rows(WW_LINECNT)("DELFLG")
        ...
        '○ 選択明細の状態を設定
        Select Case OIM0014tbl.Rows(WW_LINECNT)("OPERATION")
            Case C_LIST_OPERATION_CODE.NODATA
                OIM0014tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.NODISP
                OIM0014tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.SELECTED
                OIM0014tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.UPDATING
                OIM0014tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.UPDATING
            Case C_LIST_OPERATION_CODE.ERRORED
                OIM0014tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.ERRORED
        End Select
    ...
    Protected Sub WF_FILEUPLOAD()
        ...
        '○ XLSUPLOAD明細⇒INPtbl
        Master.CreateEmptyTable(OIM0014INPtbl)
        ...
        For Each XLSTBLrow As DataRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            If WW_COLUMNS.IndexOf("PLANTCODE") >= 0 AndAlso
                ...
                WW_COLUMNS.IndexOf("CHECKOILCODE") >= 0 AndAlso
                WW_COLUMNS.IndexOf("TANKCOUNT") >= 0 AndAlso
                WW_COLUMNS.IndexOf("DELFLG") >= 0 Then
                For Each OIM0014row As DataRow In OIM0014tbl.Rows
                    If XLSTBLrow("PLANTCODE") = OIM0014row("PLANTCODE") AndAlso
                        XLSTBLrow("BIGOILCODE") = OIM0014row("BIGOILCODE") AndAlso
                        XLSTBLrow("CHECKOILCODE") = OIM0014row("CHECKOILCODE") AndAlso
                        XLSTBLrow("TANKCOUNT") = OIM0014row("TANKCOUNT") AndAlso
                        XLSTBLrow("DELFLG") = OIM0014row("DELFLG") Then
                        OIM0014INProw.ItemArray = OIM0014row.ItemArray
                        Exit For
                    End If
                Next
            End If
            ...
            If WW_COLUMNS.IndexOf("PLANTCODE") >= 0 Then
                OIM0014INProw("PLANTCODE") = XLSTBLrow("PLANTCODE")
            End If

            '油種大分類コード
            If WW_COLUMNS.IndexOf("BIGOILCODE") >= 0 Then
                OIM0014INProw("BIGOILCODE") = XLSTBLrow("BIGOILCODE")
            End If

            '積込チェック用油種コード
            If WW_COLUMNS.IndexOf("CHECKOILCODE") >= 0 Then
                OIM0014INProw("CHECKOILCODE") = XLSTBLrow("CHECKOILCODE")
            End If

            '車数
            If WW_COLUMNS.IndexOf("TANKCOUNT") >= 0 Then
                OIM0014INProw("TANKCOUNT") = XLSTBLrow("TANKCOUNT")
            Else
                OIM0014INProw("TANKCOUNT") = "0"
            End If

            '削除フラグ
            If WW_COLUMNS.IndexOf("DELFLG") >= 0 Then
                OIM0014INProw("DELFLG") = XLSTBLrow("DELFLG")
            Else
                OIM0014INProw("DELFLG") = "0"
            End If
