...
Public Class OIM0013LoadList
        ...
        Try
            If IsPostBack Then
                ...
                If Not String.IsNullOrEmpty(WF_ButtonClick.Value) Then
                    '○ 画面表示データ復元
                    Master.RecoverTable(OIM0013tbl)
    ...
    Protected Sub Initialize()
        ...
        '○画面ID設定
        Master.MAPID = OIM0013WRKINC.MAPIDL
        '○HELP表示有無設定
    ...
    Protected Sub WW_MAPValueSet()
        ...
        '○ 検索画面からの遷移
        If Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0013S Then
            'Grid情報保存先のファイル名
            Master.CreateXMLSaveFile()
        ElseIf Context.Handler.ToString().ToUpper() = C_PREV_MAP_LIST.OIM0013C Then
    ...
    Protected Sub GridViewInitialize()
        ...
        If Context.Handler.ToString().ToUpper() <> C_PREV_MAP_LIST.OIM0013C Then
            '○ 画面表示データ取得
        ...
        '○ 画面表示データ保存
        Master.SaveTable(OIM0013tbl)

        '〇 一覧の件数を取得
        Me.WF_ListCNT.Text = "件数：" + OIM0013tbl.Rows.Count.ToString()

        '○ 一覧表示データ編集(性能対策)
        Dim TBLview As DataView = New DataView(OIM0013tbl)
    ...
    Protected Sub MAPDataGet(ByVal SQLcon As SqlConnection)
        ...
        If IsNothing(OIM0013tbl) Then
            OIM0013tbl = New DataTable
        End If

        If OIM0013tbl.Columns.Count <> 0 Then
            OIM0013tbl.Columns.Clear()
        End If
        ...
        Dim SQLStr As String =
            ...
            & "   0                                                            AS LINECNT " _
            & " , ''                                                           AS OPERATION " _
            & " , CAST(OIM0013.UPDTIMSTP AS bigint)                            AS UPDTIMSTP " _
            & " , 1                                                            AS 'SELECT' " _
            & " , 0                                                            AS HIDDEN " _
            & " , ISNULL(RTRIM(OIM0013.DELFLG), '')                            AS DELFLG " _
            & " , ISNULL(RTRIM(OIM0013.PLANTCODE), '')                         AS PLANTCODE " _
            & " , ISNULL(RTRIM(OIM0013.MAXLINECNT), '')                        AS MAXLINECNT " _
            & " , ISNULL(RTRIM(OIM0013.LOADINGPOINT), '')                      AS LOADINGPOINT " _
            & " , ISNULL(RTRIM(OIM0013.OILCODE), '')                           AS OILCODE " _
            & " , ISNULL(RTRIM(OIM0013.SEGMENTOILCODE), '')                    AS SEGMENTOILCODE " _
            & " FROM OIL.OIM0013_LOAD OIM0013 " _
            & " WHERE OIM0013.DELFLG <> @P1 "

        '○ 条件指定で指定されたものでSQLで可能なものを追加する
        '基地コード
        If Not String.IsNullOrEmpty(work.WF_SEL_PLANTCODE.Text) Then
            SQLStr &= "    AND OIM0013.PLANTCODE      = @P2"
        End If

        '積込ポイント
        If Not String.IsNullOrEmpty(work.WF_SEL_LOADINGPOINT.Text) Then
            SQLStr &= "    AND OIM0013.LOADINGPOINT   = @P3"
        End If

        '油種コード
        If Not String.IsNullOrEmpty(work.WF_SEL_OILCODE.Text) Then
            SQLStr &= "    AND OIM0013.OILCODE        = @P4"
        End If

        '油種細分コード
        If Not String.IsNullOrEmpty(work.WF_SEL_SEGMENTOILCODE.Text) Then
            SQLStr &= "    AND OIM0013.SEGMENTOILCODE = @P5"
        End If

        SQLStr &=
              " ORDER BY" _
            & "    RIGHT('0000' + CAST(OIM0013.PLANTCODE AS NVARCHAR), 4)"
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                ...
                If Not String.IsNullOrEmpty(work.WF_SEL_PLANTCODE.Text) Then
                    Dim PARA2 As SqlParameter = SQLcmd.Parameters.Add("@P2", SqlDbType.NVarChar, 4)     '基地コード
                    PARA2.Value = work.WF_SEL_PLANTCODE.Text
                End If

                If Not String.IsNullOrEmpty(work.WF_SEL_LOADINGPOINT.Text) Then
                    Dim PARA3 As SqlParameter = SQLcmd.Parameters.Add("@P3", SqlDbType.Int)             '積込ポイント
                    PARA3.Value = work.WF_SEL_LOADINGPOINT.Text
                End If
                ...
                If Not String.IsNullOrEmpty(work.WF_SEL_SEGMENTOILCODE.Text) Then
                    ...
                    PARA5.Value = work.WF_SEL_SEGMENTOILCODE.Text
                End If

                Using SQLdr As SqlDataReader = SQLcmd.ExecuteReader()
                    '○ フィールド名とフィールドの型を取得
                    For index As Integer = 0 To SQLdr.FieldCount - 1
                        OIM0013tbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                    Next

                    '○ テーブル検索結果をテーブル格納
                    OIM0013tbl.Load(SQLdr)
                End Using

                Dim i As Integer = 0
                For Each OIM0013row As DataRow In OIM0013tbl.Rows
                    i += 1
                    OIM0013row("LINECNT") = i        'LINECNT
                Next
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0013L SELECT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                         'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0013L Select"
    ...
    Protected Sub DisplayGrid()
        ...
        '○ 表示対象行カウント(絞り込み対象)
        For Each OIM0013row As DataRow In OIM0013tbl.Rows
            If OIM0013row("HIDDEN") = 0 Then
                WW_DataCNT += 1
                '行(LINECNT)を再設定する。既存項目(SELECT)を利用
                OIM0013row("SELECT") = WW_DataCNT
            End If
        Next
    ...
    Protected Sub WF_ButtonINSERT_Click()
        ...
        '最大回線数
        work.WF_SEL_MAXLINECNT.Text = ""

        '積込ポイント(登録(新規追加用))
        work.WF_SEL_LOADINGPOINT2.Text = ""
    ...
    Protected Sub UpdateMaster(ByVal SQLcon As SqlConnection)
        ...
        Dim SQLStr As String =
            ...
            & "    WHERE" _
            & "        PLANTCODE      = @P01 " _
            & "    AND MAXLINECNT     = @P02 " _
            & "    AND LOADINGPOINT   = @P03 " _
            & "    AND OILCODE        = @P04 " _
            & "    AND SEGMENTOILCODE = @P05 ;" _
            & " OPEN hensuu ;" _
            & " FETCH NEXT FROM hensuu INTO @hensuu ;" _
            & " IF (@@FETCH_STATUS = 0)" _
            & "    UPDATE OIL.OIM0013_LOAD" _
            & "    SET" _
            & "        DELFLG = @P00" _
            & "    WHERE" _
            & "        PLANTCODE      = @P01 " _
            ...
            & "    AND SEGMENTOILCODE = @P05 ;" _
            & " IF (@@FETCH_STATUS <> 0)" _
            & "    INSERT INTO OIL.OIM0013_LOAD" _
            & "        (DELFLG" _
            & "        , PLANTCODE" _
            & "        , MAXLINECNT" _
            & "        , LOADINGPOINT" _
        ...
        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon), SQLcmdJnl As New SqlCommand(SQLJnl, SQLcon)
                ...
                Dim PARA02 As SqlParameter = SQLcmd.Parameters.Add("@P02", SqlDbType.Int)                   '最大回線数
                Dim PARA03 As SqlParameter = SQLcmd.Parameters.Add("@P03", SqlDbType.Int)                   '積込ポイント
                ...
                Dim JPARA02 As SqlParameter = SQLcmdJnl.Parameters.Add("@P02", SqlDbType.Int)               '最大回線数
                Dim JPARA03 As SqlParameter = SQLcmdJnl.Parameters.Add("@P03", SqlDbType.Int)               '積込ポイント
                ...
                For Each OIM0013row As DataRow In OIM0013tbl.Rows
                    If Trim(OIM0013row("OPERATION")) = C_LIST_OPERATION_CODE.UPDATING OrElse
                        Trim(OIM0013row("OPERATION")) = C_LIST_OPERATION_CODE.INSERTING OrElse
                        Trim(OIM0013row("OPERATION")) = C_LIST_OPERATION_CODE.SELECTED Then
                        Dim WW_DATENOW As DateTime = Date.Now

                        'DB更新
                        PARA00.Value = OIM0013row("DELFLG")
                        PARA01.Value = OIM0013row("PLANTCODE")
                        PARA02.Value = OIM0013row("MAXLINECNT")
                        PARA03.Value = OIM0013row("LOADINGPOINT")
                        PARA04.Value = OIM0013row("OILCODE")
                        PARA05.Value = OIM0013row("SEGMENTOILCODE")
                        ...
                        '更新ジャーナル出力
                        JPARA00.Value = OIM0013row("DELFLG")
                        JPARA01.Value = OIM0013row("PLANTCODE")
                        JPARA02.Value = OIM0013row("MAXLINECNT")
                        JPARA03.Value = OIM0013row("LOADINGPOINT")
                        JPARA04.Value = OIM0013row("OILCODE")
                        JPARA05.Value = OIM0013row("SEGMENTOILCODE")

                        Using SQLdr As SqlDataReader = SQLcmdJnl.ExecuteReader()
                            If IsNothing(OIM0013UPDtbl) Then
                                OIM0013UPDtbl = New DataTable

                                For index As Integer = 0 To SQLdr.FieldCount - 1
                                    OIM0013UPDtbl.Columns.Add(SQLdr.GetName(index), SQLdr.GetFieldType(index))
                                Next
                            End If

                            OIM0013UPDtbl.Clear()
                            OIM0013UPDtbl.Load(SQLdr)
                        End Using

                        For Each OIM0013UPDrow As DataRow In OIM0013UPDtbl.Rows
                            CS0020JOURNAL.TABLENM = "OIM0013L"
                            CS0020JOURNAL.ACTION = "UPDATE_INSERT"
                            CS0020JOURNAL.ROW = OIM0013UPDrow
            ...
            End Using
        Catch ex As Exception
            Master.Output(C_MESSAGE_NO.DB_ERROR, C_MESSAGE_TYPE.ABORT, "OIM0013L UPDATE_INSERT")

            CS0011LOGWrite.INFSUBCLASS = "MAIN"                             'SUBクラス名
            CS0011LOGWrite.INFPOSI = "DB:OIM0013L UPDATE_INSERT"
            CS0011LOGWrite.NIWEA = C_MESSAGE_TYPE.ABORT
            CS0011LOGWrite.TEXT = ex.ToString()
            CS0011LOGWrite.MESSAGENO = C_MESSAGE_NO.DB_ERROR
            CS0011LOGWrite.CS0011LOGWrite()                                 'ログ出力
    ...
    Protected Sub WF_ButtonDownload_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0013tbl                        'データ参照  Table
    ...
    Protected Sub WF_ButtonPrint_Click()
        ...
        CS0030REPORT.TBLDATA = OIM0013tbl                        'データ参照Table
        CS0030REPORT.CS0030REPORT()
        If Not isNormal(CS0030REPORT.ERR) Then
            If CS0030REPORT.ERR = C_MESSAGE_NO.REPORT_EXCEL_NOT_FOUND_ERROR Then
                Master.Output(CS0030REPORT.ERR, C_MESSAGE_TYPE.ERR, needsPopUp:=True)
    ...
    Protected Sub WF_ButtonLAST_Click()
        ...
        '○ ソート
        Dim TBLview As New DataView(OIM0013tbl)
    ...
    Protected Sub WF_Grid_DBClick()
        ...
        '選択行
        work.WF_SEL_LINECNT.Text = OIM0013tbl.Rows(WW_LINECNT)("LINECNT")

        '荷受人コード
        work.WF_SEL_PLANTCODE2.Text = OIM0013tbl.Rows(WW_LINECNT)("PLANTCODE")

        '最大回線数
        work.WF_SEL_MAXLINECNT.Text = OIM0013tbl.Rows(WW_LINECNT)("MAXLINECNT")

        '積込ポイント
        work.WF_SEL_LOADINGPOINT2.Text = OIM0013tbl.Rows(WW_LINECNT)("LOADINGPOINT")

        '油種コード
        work.WF_SEL_OILCODE2.Text = OIM0013tbl.Rows(WW_LINECNT)("OILCODE")

        '油種細分コード
        work.WF_SEL_SEGMENTOILCODE2.Text = OIM0013tbl.Rows(WW_LINECNT)("SEGMENTOILCODE")

        '削除フラグ
        work.WF_SEL_DELFLG.Text = OIM0013tbl.Rows(WW_LINECNT)("DELFLG")
        ...
        '○ 選択明細の状態を設定
        Select Case OIM0013tbl.Rows(WW_LINECNT)("OPERATION")
            Case C_LIST_OPERATION_CODE.NODATA
                OIM0013tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.NODISP
                OIM0013tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.SELECTED
                OIM0013tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED
            Case C_LIST_OPERATION_CODE.UPDATING
                OIM0013tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.UPDATING
            Case C_LIST_OPERATION_CODE.ERRORED
                OIM0013tbl.Rows(WW_LINECNT)("OPERATION") = C_LIST_OPERATION_CODE.SELECTED & C_LIST_OPERATION_CODE.ERRORED
        End Select
    ...
    Protected Sub WF_FILEUPLOAD()
        ...
        '○ XLSUPLOAD明細⇒INPtbl
        Master.CreateEmptyTable(OIM0013INPtbl)
        ...
        For Each XLSTBLrow As DataRow In CS0023XLSUPLOAD.TBLDATA.Rows
            ...
            If WW_COLUMNS.IndexOf("PLANTCODE") >= 0 AndAlso
                WW_COLUMNS.IndexOf("MAXLINECNT") >= 0 AndAlso
                WW_COLUMNS.IndexOf("LOADINGPOINT") >= 0 AndAlso
                WW_COLUMNS.IndexOf("OILCODE") >= 0 AndAlso
                WW_COLUMNS.IndexOf("SEGMENTOILCODE") >= 0 AndAlso
                WW_COLUMNS.IndexOf("DELFLG") >= 0 Then
                For Each OIM0013row As DataRow In OIM0013tbl.Rows
                    If XLSTBLrow("PLANTCODE") = OIM0013row("PLANTCODE") AndAlso
                        XLSTBLrow("MAXLINECNT") = OIM0013row("MAXLINECNT") AndAlso
                        XLSTBLrow("LOADINGPOINT") = OIM0013row("LOADINGPOINT") AndAlso
                        XLSTBLrow("OILCODE") = OIM0013row("OILCODE") AndAlso
                        XLSTBLrow("SEGMENTOILCODE") = OIM0013row("SEGMENTOILCODE") AndAlso
                        XLSTBLrow("DELFLG") = OIM0013row("DELFLG") Then
                        OIM0013INProw.ItemArray = OIM0013row.ItemArray
                        Exit For
                    End If
                Next
            End If
            ...
            If WW_COLUMNS.IndexOf("PLANTCODE") >= 0 Then
                OIM0013INProw("PLANTCODE") = XLSTBLrow("PLANTCODE")
            End If

            '最大回線数
            If WW_COLUMNS.IndexOf("MAXLINECNT") >= 0 Then
                OIM0013INProw("MAXLINECNT") = XLSTBLrow("MAXLINECNT")
            Else
                OIM0013INProw("MAXLINECNT") = "0"
            End If

            '積込ポイント
            If WW_COLUMNS.IndexOf("LOADINGPOINT") >= 0 Then
                OIM0013INProw("LOADINGPOINT") = XLSTBLrow("LOADINGPOINT")
            Else
                OIM0013INProw("LOADINGPOINT") = "0"
            End If

            '油種コード
            If WW_COLUMNS.IndexOf("OILCODE") >= 0 Then
                OIM0013INProw("OILCODE") = XLSTBLrow("OILCODE")
            End If

            '油種細分コード
            If WW_COLUMNS.IndexOf("SEGMENTOILCODE") >= 0 Then
                OIM0013INProw("SEGMENTOILCODE") = XLSTBLrow("SEGMENTOILCODE")
            End If

            '削除フラグ
            If WW_COLUMNS.IndexOf("DELFLG") >= 0 Then
                OIM0013INProw("DELFLG") = XLSTBLrow("DELFLG")
            Else
                OIM0013INProw("DELFLG") = "0"
            End If
        ...
        For Each OIM0013INProw As DataRow In OIM0013INPtbl.Rows
            ...
            For Each OIM0013row As DataRow In OIM0013tbl.Rows
                If OIM0013row("PLANTCODE") = OIM0013INProw("PLANTCODE") AndAlso
                    ...
                    If OIM0013row("MAXLINECNT") = OIM0013row("MAXLINECNT") AndAlso
                        OIM0013row("LOADINGPOINT") = OIM0013row("LOADINGPOINT") AndAlso
                        OIM0013row("OILCODE") = OIM0013row("OILCODE") AndAlso
                        OIM0013row("SEGMENTOILCODE") = OIM0013row("SEGMENTOILCODE") AndAlso
                        OIM0013row("DELFLG") = OIM0013INProw("DELFLG") AndAlso
