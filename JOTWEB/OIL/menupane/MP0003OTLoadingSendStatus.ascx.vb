...
''' OT発送日報送信状況コントロールクラス
''' </summary>
Public Class MP0003OTLoadingSendStatus
    Inherits MP0000Base
    '*共通関数宣言(BASEDLL)
    ...
    Protected Sub Initialize()
        Me.lblPaneTitle.Text = String.Format("{0:M月d日} OT発送日報送信状況", Now)
        'MP0000Baseの共通処理の営業所抽出を呼出し営業所ドロップダウン生成
        Dim retDdl As DropDownList = Me.GetOfficeList()
        If retDdl.Items.Count > 0 Then
            Me.ddlLoadingSendStatusOffice.Items.AddRange(retDdl.Items.Cast(Of ListItem).ToArray)
            Me.ddlLoadingSendStatusOffice.SelectedIndex = retDdl.SelectedIndex
        End If
        SetDisplayValues()
    End Sub
    ''' <summary>
    ...
    Private Function GetBaseTrainList(sqlCon As SqlConnection) As List(Of LinkImportItem)
        ...
        Using sqlCmd As New SqlCommand(sqlStat.ToString, sqlCon)
            With sqlCmd.Parameters
                .Add("@OFFICECODE", SqlDbType.NVarChar).Value = Me.ddlLoadingSendStatusOffice.SelectedValue
    ...
    Private Sub SetDisplayValues()
        ...
        With Me.repLoadingSendStatusItems
            .DataSource = dispList
            .DataBind()
        End With
