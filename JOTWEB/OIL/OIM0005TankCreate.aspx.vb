...
Public Class OIM0005TankCreate
    ...
    Protected Sub UniqueKeyCheck(ByVal SQLcon As SqlConnection, ByRef O_MESSAGENO As String)
        ...
        ''○ 条件指定で指定されたものでSQLで可能なものを追加する
        ''貨物コード枝番
        'If Not String.IsNullOrEmpty(WF_MODEL.Text) Then
        '    SQLStr &= String.Format("    AND MODEL = '{0}'", WF_MODEL.Text)
        'Else
        '    SQLStr &= String.Format("    AND MODEL = '{0}'", "")
        'End If

        Try
            Using SQLcmd As New SqlCommand(SQLStr, SQLcon)
                Dim PARA1 As SqlParameter = SQLcmd.Parameters.Add("@P01", SqlDbType.NVarChar, 4)            'JOT車番
                ...
                For Each OIM0005row As DataRow In OIM0005tbl.Rows
                    If Trim(OIM0005row("OPERATION")) = C_LIST_OPERATION_CODE.UPDATING OrElse
                        ...
                        For Each OIM0005UPDrow As DataRow In OIM0005UPDtbl.Rows
                            CS0020JOURNAL.TABLENM = "OIM0005C"
